...
export function FloatingCreateDock({ children }: FloatingCreateDockProps) {
  return (
    <>
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 bottom-0 z-10 h-32 bg-gradient-to-t from-[var(--background)] via-[var(--background)]/70 to-transparent"
      />
      <div className="safe-area-pb pointer-events-auto absolute inset-x-0 bottom-0 z-20 px-4 pb-4">
        {children}
      </div>
    </>
  );
